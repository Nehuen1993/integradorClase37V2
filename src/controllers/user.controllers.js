...
export const postRegister = async (req, res) => {
    ...
    const newUser = new UserDTO (first_name, last_name, email, age, password, telefono );
    console.log (newUser)
    const user = await userServices.create(newUser);

    console.log('Usuario registrado con éxito.' + user);
    res.redirect('/api/sessions/login').status(200).send({ status: 'success', message: 'usuario registrado con éxito'}).
